   ...
   <div class="container mt-5" >
    ...
    <div class="card body">
       ...
       if (isset($_GET['id'])){
        ...
        $sql = "SELECT * FROM cliente WHERE idcliente = '$cliente_id'"; 
        $query = mysqli_query ($conex, $sql);

        if (mysqli_num_rows($query) > 0) {
            $cliente = mysqli_fetch_array($query);
       ?> 
    <form action="acoes3.php" method="POST">
        <input type="hidden" name="cliente_id"   value="<?=$cliente['idcliente']?>">
    <div class="mb-3">
    <label>Nome do cliente</label>
    <input type="text" name="nome_clien" value="<?=$cliente['nome_clien']?>" class="form-control">
    </div>
    <div class="mb-3">
    <label>Email</label>
    ...
    <input type="text" name="endere" 
           value="<?= htmlspecialchars($cliente['endere'] ?? '', ENT_QUOTES, 'UTF-8') ?>" 
           class="form-control">
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
<button type="submit" name="update_usuario3" class="btn btn-primary">Salvar</button>
    </div>
    </form>
    <?php 
       } else{
